...
export function SnackCard({ snack, variant = 'grid' }: SnackCardProps) {
  ...
  const allDisplayTags = [
    ...snack.tags.slice(0, 2).map((tagId) => {
      ...
      return info
        ? { name: info.name, color: info.color, bgColor: info.bgColor, isCustom: false }
        : null
    }),
    ...snackCustomTags
      .slice(0, 1)
      .map((tag) =>
        tag
          ? { name: tag.name, color: '', bgColor: '', isCustom: true, tagColor: tag.color }
          : null,
      ),
  ].filter(Boolean)
      ...
      <div className="flex items-center justify-between mt-3">
        ...
        <span
          className={`text-xs font-medium px-2 py-1 rounded-full ${
            caloriesLevel.level === '低'
              ? 'bg-green-100 text-green-600'
              : caloriesLevel.level === '中低'
                ? 'bg-emerald-100 text-emerald-600'
                : caloriesLevel.level === '中'
                  ? 'bg-yellow-100 text-yellow-600'
                  : caloriesLevel.level === '高'
