...
export function CalendarHeatmap({
  ...
  const calendarDays = useMemo(() => {
    ...
    const prevMonthLastDay = new Date(currentMonth.year, currentMonth.month, 0).getDate()
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-lg text-gray-800">日历热力图</h3>
        </div>
        <div className="flex items-center gap-2">
          <button
        ...
        {calendarDays.map((dayInfo, index) => {
          ...
          return (
            <button
              ...
              className={`relative aspect-square rounded-lg flex items-center justify-center text-sm font-medium transition-all ${getHeatmapColor(
                dayInfo.date,
                ...
                isToday(dayInfo.date) && dayInfo.isCurrentMonth ? 'ring-2 ring-accent-500' : ''
              } ${dayInfo.isCurrentMonth ? 'cursor-pointer' : 'cursor-default'}`}
              title={dayInfo.isCurrentMonth ? `${dayInfo.date}: ${calories} kcal` : ''}
            >
              {dayInfo.day}
      ...
      <div className="mt-6 flex items-center justify-center gap-4">
        ...
        <div className="flex items-center gap-1">
          {[0, 1, 2, 3, 4].map((level) => {
            ...
            return <div key={level} className={`w-6 h-6 rounded ${colors[level]}`} />
          })}
        </div>
