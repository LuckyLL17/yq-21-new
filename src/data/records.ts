...
export function updateRecord(id: string, updates: Partial<CalorieRecord>): CalorieRecord | null {
  const records = getRecords()
...
export function getRecordsByDateRange(startDate: string, endDate: string): CalorieRecord[] {
  const records = getRecords()
...
export function getCalendarHeatmapData(year: number, month: number): Map<string, number> {
...
export function getHeatmapLevel(calories: number, maxCalories: number = 2000): number {
