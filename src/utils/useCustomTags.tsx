...
export function CustomTagsProvider({ children }: { children: ReactNode }) {
  ...
  const addCustomTag = useCallback((name: string, color?: string): CustomTag => {
  ...
  const getSnackTags = useCallback((snackId: string): string[] => {
  ...
  const hasTagOnSnack = useCallback((snackId: string, tagId: string): boolean => {
    return snackTagsMap[snackId]?.includes(tagId) || false;
  }, [snackTagsMap]);

  const getSnacksByTag = useCallback((tagId: string): string[] => {
